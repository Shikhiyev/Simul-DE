    ...
    while (size) {
        ...
        switch (tag) {
                ...
                for (int i = 0; i < 8; i++)
                    if (!firmware->external_state[i].port) {
                        ...
                        AVR_LOG(NULL, LOG_TRACE,
                                "AVR_MMCU_TAG_PORT_EXTERNAL_PULL[%d] "
                                "%c:%02x:%02x\n",
            ...
            case AVR_MMCU_TAG_VCD_TRACE: {
                ...
                // #if 0
                AVR_LOG(NULL, LOG_DEBUG, "VCD_TRACE %d %04x:%02x - %s\n", tag,
                        addr, mask, name);
                // #endif
...
int elf_read_firmware_ext(const char *file, elf_firmware_t *firmware)
{
        ...
        if (shdr.sh_type == SHT_SYMTAB) {
            ...
            for (int i = 0; i < symbol_count; i++) {
                ...
                if (ELF32_ST_BIND(sym.st_info) == STB_GLOBAL ||
                    ...
                    // if its a bootloader, this symbol will be the entry point
                    // we need
                    if (!strcmp(name, "__vectors"))
                        firmware->flashbase = sym.st_value;
                    avr_symbol_t *s =
                        malloc(sizeof(avr_symbol_t) + strlen(name) + 1);
                    strcpy((char *)s->symbol, name);
                    s->addr = sym.st_value;
                    if (!(firmware->symbolcount % 8))
                        firmware->symbol = realloc(
                            firmware->symbol, (firmware->symbolcount + 8) *
    ...
    uint32_t offset     = 0;
    firmware->flashsize = (data_text ? data_text->d_size : 0) +
    ...
    /*
        ...
        //	hdump("code", data_text->d_buf, data_text->d_size);
                memcpy(firmware->flash + offset, data_text->d_buf,
       data_text->d_size); AVR_LOG(NULL, LOG_TRACE, "Loaded %u .text at address
       0x%x\n", (unsigned int)data_text->d_size, firmware->flashbase); offset +=
       data_text->d_size;
        }
        if (data_data) {
        //	hdump("data", data_data->d_buf, data_data->d_size);
                memcpy(firmware->flash + offset, data_data->d_buf,
       data_data->d_size); AVR_LOG(NULL, LOG_TRACE, "Loaded %u .data\n",
       (unsigned int)data_data->d_size); offset += data_data->d_size;
    ...
    */
    ...
    for (int ii = 0; ii < section_no; ii++) {
        if (((uint32_t)flash_sections[ii].shdr.sh_addr & 0x800000) != 0) {
            // 0x800000 bit mean this is a "Harvard architecture" data section:
            // skip it as it is already calculated.
            continue;
        }
        uint32_t size = (uint32_t)flash_sections[ii].shdr.sh_addr +
                        (uint32_t)flash_sections[ii].shdr.sh_size;
    ...
    for (int ii = 0; ii < section_no; ii++) {
        if (flash_sections[ii].data_ptr == data_text) {
            /*memcpy(firmware->flash + offset, data_text->d_buf,
            data_text->d_size); AVR_LOG(NULL, LOG_TRACE, "Loaded %u .text at
            address 0x%x\n", (unsigned int)data_text->d_size,
            firmware->flashbase); offset += data_text->d_size;*/
            continue;
            ...
            // This is a custom flash section. The shdr.sh_addr should contain
            // its actual address.
            memcpy(firmware->flash + (uint32_t)flash_sections[ii].shdr.sh_addr,
                   flash_sections[ii].data_ptr->d_buf,
                   (uint32_t)flash_sections[ii].shdr.sh_size);
        }
    }
