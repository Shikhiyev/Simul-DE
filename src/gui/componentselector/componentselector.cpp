...
ComponentSelector::ComponentSelector(QWidget *parent) :
    QTreeWidget(parent), m_pluginsdDialog(this), m_itemLibrary()
{
    m_pSelf = this;
    ...
    // QWhatsThis::add( this, QString::fromUtf8( gettext("Drag a Component and
    // drop it into the Circuit." ) ) );
        ...
        if (parent) {
            while (!node.isNull()) {
                ...
                if (element.hasAttribute("icon")) {
                    ...
                    icon =
                        compSetDir.absoluteFilePath(element.attribute("icon"));
                }
                QString name = element.attribute("name");

                m_xmlFileList[name] =
                    setFile; // Save xml File used to create this item

                if (element.hasAttribute("info"))
                    name += "???" + element.attribute("info");
...
void ComponentSelector::addItem(const QString &caption,
                                const QString &_category, const QString &icon,
                                const QString &type)
{
    ...
    bool hidden =
        MainWindow::self()->settings()->value(name + "/hidden").toBool();
    ...
    if (!m_categories.contains(category,
                               Qt::CaseSensitive)) // Create new Category
    {
        bool c_hidden = false;
        bool expanded = false;
        ...
        if (isRootCat) // Is Main Category
        {
            ...
            catItem->setTextColor(
                0, QColor(110, 95, 50) /*QColor(255, 230, 200)*/);
        ...
        if (!catPath.isEmpty()) {
            ...
            QList<QTreeWidgetItem *> list =
                findItems(topCat, Qt::MatchExactly | Qt::MatchRecursive);
    ...
    {
        QList<QTreeWidgetItem *> list =
            findItems(category, Qt::MatchExactly | Qt::MatchRecursive);
    ...
    if (type == "")
        ...
        font.setPixelSize(11 * fontScale);
    ...
    if ((type == "Subcircuit") || (type == "PIC") || (type == "AVR") ||
        (type == "Arduino")) {
        item->setData(0, Qt::WhatsThisRole, name);
    } else
        item->setData(0, Qt::WhatsThisRole, type);

    catItem->addChild(item);
    item->setHidden(hidden);
    if (MainWindow::self()->settings()->contains(name + "/collapsed")) {
        bool expanded = !MainWindow::self()
                             ->settings()
...
void ComponentSelector::slotItemClicked(QTreeWidgetItem *item, int column)
{
    ...
    QString name =
        item->data(0, Qt::WhatsThisRole).toString(); // item->text(0);
...
void ComponentSelector::slotContextMenu(const QPoint &point)
{
    ...
    QAction *managePluginAction =
        menu.addAction(QIcon(":/fileopen.png"), tr("Manage Components"));
    connect(managePluginAction, SIGNAL(triggered()), this,
            SLOT(slotManageComponents()));

    // QAction* installAction = menu.addAction(
    // QIcon(":/fileopen.png"),tr("Install New Component") ); connect(
    // installAction, SIGNAL(triggered()), this, SLOT(slotIstallItem()) );
...
/*void ComponentSelector::unistallPlugin( QString item )
{
    ...
    if( fIn.open( QFile::ReadOnly | QFile::Text ) ) // Read unistall file
    {
        ...
        while (!sIn.atEnd())                    // Remove installed files
        {
            ...
            {
                QString pluginName =
pluginPath.split("/").last().split(".").first().remove("lib").remove("plugin").toUpper();
...
QMimeData *
ComponentSelector::mimeData(const QList<QTreeWidgetItem *> items) const
{
    if (items.isEmpty())
        return 0;
