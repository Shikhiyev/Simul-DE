...
CircuitWidget::CircuitWidget(QWidget *parent) :
    QWidget(parent), m_verticalLayout(this), m_horizontLayout(),
    m_circView(this)
    //, m_terminal(this)
    ,
    m_plotter(this), m_circToolBar(this), m_infoMenu(this)
{
    m_pSelf = this;
    ...
    m_lastCircDir =
        MainWindow::self()->settings()->value("lastCircDir").toByteArray();
    if (m_lastCircDir.isEmpty())
        m_lastCircDir = appPath + "..share/simulide/examples";
...
void CircuitWidget::createActions()
{
    ...
    powerCircAct =
        new QAction(QIcon(":/poweroff.png"), tr("Power Circuit"), this);
    powerCircAct->setStatusTip(tr("Power the Circuit"));
    connect(powerCircAct, SIGNAL(triggered()), this, SLOT(powerCirc()));

    pauseSimAct =
        new QAction(QIcon(":/pausesim.png"), tr("Pause Simulation"), this);
    ...
    darkModeAct->setChecked(MainWindow::self()->isDarkMode());
    connect(darkModeAct, SIGNAL(triggered()), this, SLOT(setDarkMode()));

    lightModeAct = new QAction(tr("Light Mode"), this);
    lightModeAct->setCheckable(true);
    lightModeAct->setChecked(!MainWindow::self()->isDarkMode());
    connect(lightModeAct, SIGNAL(triggered()), this, SLOT(setLightMode()));
...
void CircuitWidget::saveCirc()
{
    ...
    if (m_curCirc.isEmpty())
        ...
        saved = Circuit::self()->saveCircuit(m_curCirc);
...
bool CircuitWidget::saveCircAs()
{
    const QString dir = m_lastCircDir;
    QString fileName =
        QFileDialog::getSaveFileName(this, tr("Save Circuit"), dir,
...
void CircuitWidget::about()
{
    ...
    QString t = "&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;";
    QMessageBox::about(
        this, tr("About SimulIDE"),
        "<br><br>"
        "<b>Version:</b>&nbsp; &nbsp; &nbsp;" +
            QString(APP_VERSION) + "<br><b>Compiled:</b> " + date +
            "<br><br><br>"
            "<b>Web site:</b> <a href=\"https://simulide.blogspot.com/\"> "
            "https://simulide.blogspot.com/ </a><br><br>"
            "<b>Become a Patron:</b> <a "
            "href=\"https://www.patreon.com/simulide\"> "
            "https://www.patreon.com/simulide </a><br><br>"
            "<br><br>"
            "<b>Creator:</b> Santiago Gonzalez. <br>"
            "<br>"
            "<b>Developers:</b> <br>" +
            t + "Santiago Gonzalez. <br>" + t +
            "Popov Alexey <br>"
            "<br>"
            "<b>Contributors:</b> <br>" +
            t + "Chris Roper <br>" + t + "Sergei Chiyanov <br>" + t +
            "Sergey Roenko <br>" + t +
            "Gabor Nagy <br>"
            "<br>"
            "<b>Translations:</b> <br>" +
            t + "Spanish: Santiago Gonzalez. <br>" + t + t +
            "&nbsp; Kike Gl. <br>" + t + "Russian: Sergei Chiyanov <br>" + t +
            "French:  Pascal Cotret <br>");
}

void CircuitWidget::setRate(int rate)
{
    if (rate < 0)
        m_rateLabel->setText(tr("Circuit ERROR!!!"));
    else
        m_rateLabel->setText(tr("    Real Speed: ") + QString::number(rate) +
                             " %");
}

void CircuitWidget::setDarkMode()
{
    MainWindow::self()->setTheme(true);
    darkModeAct->setChecked(true);
    lightModeAct->setChecked(false);
}

void CircuitWidget::setLightMode()
{
