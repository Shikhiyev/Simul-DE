...
void CircuitView::setCircTime(uint64_t step)
{
    ...
    m_info->setPlainText(tr("Time: ") + strH + ":" + strM + ":" + strS + "." +
                         strMS + strMcu);
}
...
void CircuitView::wheelEvent(QWheelEvent *event)
{
    ...
    if (newScale < 0.1) {
        ...
        scaleFactor = 100.0 / currentScale;
    }
...
void CircuitView::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    m_enterItem =
        m_circuit->createItem(type, id + "-" + m_circuit->newSceneId());
    if (m_enterItem) {
...
void CircuitView::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
    if (m_enterItem)
        m_enterItem->moveTo(togrid(mapToScene(event->pos())));
}
...
void CircuitView::resizeEvent(QResizeEvent *event)
{
    ...
    m_circuit->setSceneRect(-width / 2 + 2, -height / 2 + 2, width - 4,
                            height - 4);
...
void CircuitView::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (m_circuit->is_constarted())
        ...
        QAction *pasteAction =
            menu.addAction(QIcon(":/paste.png"), tr("Paste") + "\tCtrl+V");
        connect(pasteAction, SIGNAL(triggered()), this, SLOT(slotPaste()));

        QAction *undoAction =
            menu.addAction(QIcon(":/undo.png"), tr("Undo") + "\tCtrl+Z");
        connect(undoAction, SIGNAL(triggered()), Circuit::self(), SLOT(undo()));

        QAction *redoAction =
            menu.addAction(QIcon(":/redo.png"), tr("Redo") + "\tCtrl+Y");
        connect(redoAction, SIGNAL(triggered()), Circuit::self(), SLOT(redo()));
        menu.addSeparator();

        /*QAction* openCircAct = menu.addAction(QIcon(":/opencirc.png"),
        tr("Open Circuit")+"\tCtrl+O" ); connect(openCircAct,
        SIGNAL(triggered()), CircuitWidget::self(), SLOT(openCirc()));

        QAction* newCircAct = menu.addAction( QIcon(":/newcirc.png"), tr("New
        Circuit")+"\tCtrl+N" ); connect( newCircAct, SIGNAL(triggered()),
        CircuitWidget::self(), SLOT(newCircuit()));

        QAction* saveCircAct = menu.addAction(QIcon(":/savecirc.png"), tr("Save
        Circuit")+"\tCtrl+S" ); connect(saveCircAct, SIGNAL(triggered()),
        CircuitWidget::self(), SLOT(saveCirc()));

        QAction* saveCircAsAct =
        menu.addAction(QIcon(":/savecircas.png"),tr("Save Circuit
        As...")+"\tCtrl+Shift+S" ); connect(saveCircAsAct, SIGNAL(triggered()),
        CircuitWidget::self(), SLOT(saveCircAs())); menu.addSeparator();*/

        QAction *importCircAct =
            menu.addAction(QIcon(":/opencirc.png"), tr("Import Circuit"));
        connect(importCircAct, SIGNAL(triggered()), this, SLOT(importCirc()));

        QAction *saveImgAct = menu.addAction(QIcon(":/saveimage.png"),
                                             tr("Save Circuit as Image"));
        connect(saveImgAct, SIGNAL(triggered()), this, SLOT(saveImage()));

        QAction *createSubCircAct =
            menu.addAction(QIcon(":/load.png"), tr("Create SubCircuit"));
        connect(createSubCircAct, SIGNAL(triggered()), Circuit::self(),
                SLOT(createSubcircuit()));

        QAction *createBomAct =
            menu.addAction(QIcon(":/savecirc.png"), tr("Bill of Materials"));
        connect(createBomAct, SIGNAL(triggered()), Circuit::self(),
                SLOT(bom()));
...
void CircuitView::saveImage()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Save as Image"), circPath,
