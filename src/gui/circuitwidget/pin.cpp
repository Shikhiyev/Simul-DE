...
Pin::Pin(int angle, const QPoint &pos, QString id, int index,
         Component *parent) :
...
void Pin::reset()
{
    ...
    // if( !Circuit::self()->deleting() )
    {
        ...
        if (m_isBus)
            m_component->inStateChanged(3); // Used by Bus to remove
    }
    ePin::reset();
}
...
void Pin::registerPinsW(
    eNode *enode) // Called by connector closing or other pin
{
    if (m_blocked)
        return;
    m_blocked = true;
    ...
    if (m_conPin)
        m_conPin->registerPins(enode); // Call pin at other side of Connector
...
void Pin::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (event->button() == Qt::LeftButton) {
        if (my_connector == 0l) {
            ...
            event->accept();
            if (Circuit::self()->is_constarted())
                Circuit::self()->closeconnector(this);
            else
                Circuit::self()->newconnector(this);
        } else
            event->ignore();
                ...
                QWidget *widget)
{
    ...
    if (MainWindow::self()->isDarkMode() && penColor == Qt::black)
        penColor = Qt::white;
