...
{
    if (m_processor && m_processor->getRamTable())
...
{
    QString compName =
        m_id.split("-").first(); // for example: "atmega328-1" to: "atmega328"
    qDebug() << "McuComponent::initChip compName:" << compName;

    if (!ComponentSelector::self()) {
        qDebug() << "McuComponent::initChip CRITICAL: "
                    "ComponentSelector::self() is NULL";
        m_error = 1;
        return;
    }
    QString dataFile = ComponentSelector::self()->getXmlFile(compName);
    qDebug() << "McuComponent::initChip dataFile:" << dataFile;
    ...
    while (!rNode.isNull()) {
        ...
        while (!node.isNull()) {
            ...
            if (element.attribute("name") == compName) {
                ...
                m_pkgeFile =
                    dataDir.filePath(element.attribute("package")) + ".package";
                ...
                if (m_processor)
                    ...
                    qDebug()
                        << "McuComponent::initChip ERROR: m_processor is NULL";

                // Get data file
                QString dataFile =
                    dataDir.filePath(element.attribute("data")) + ".data";

                if (m_processor)
                    m_processor->setDataFile(dataFile);
                if (element.hasAttribute("icon"))
                    m_BackGround = ":/" + element.attribute("icon");
    ...
    if (m_device != "")
        ...
        m_error = 1;
        qDebug() << compName
                 << "ERROR!! McuComponent::initChip Chip not Found: "
                 << package;
    }
}

void McuComponent::updatePin(QString id, QString type, QString label, int pos,
                             int xpos, int ypos, int angle)
{
    Pin *pin = 0l;
    ...
    if (!pin) {
        qDebug() << "McuComponent::updatePin Pin Not Found:" << id << type
                 << label;
        return;
    }
    ...
    if (type == "gnd" || type == "vdd" || type == "vcc" || type == "unused" ||
        type == "nc")
        pin->setUnused(true);
    else
        pin->setUnused(false);
...
void McuComponent::contextMenu(QGraphicsSceneContextMenuEvent *event,
                               QMenu *menu)
{
    QAction *loadAction =
        menu->addAction(QIcon(":/load.png"), tr("Load firmware"));
    connect(loadAction, SIGNAL(triggered()), this, SLOT(slotLoad()));

    QAction *reloadAction =
        menu->addAction(QIcon(":/reload.png"), tr("Reload firmware"));
    connect(reloadAction, SIGNAL(triggered()), this, SLOT(slotReload()));

    QAction *loadDaAction =
        menu->addAction(QIcon(":/load.png"), tr("Load EEPROM data"));
    connect(loadDaAction, SIGNAL(triggered()), this, SLOT(loadData()));

    QAction *saveDaAction =
        menu->addAction(QIcon(":/save.png"), tr("Save EEPROM data"));
    connect(saveDaAction, SIGNAL(triggered()), this, SLOT(saveData()));

    menu->addSeparator();

    QAction *openTerminal =
        menu->addAction(QIcon(":/terminal.png"), tr("Open Serial Monitor."));
    connect(openTerminal, SIGNAL(triggered()), this, SLOT(slotOpenTerm()));

    QAction *openSerial =
        menu->addAction(QIcon(":/terminal.png"), tr("Open Serial Port."));
...
{
    Component *ser = Circuit::self()->createItem(
        "SerialPort", "SerialPort-" + Circuit::self()->newSceneId());
    ser->setPos(pos());
...
{
    Component *ser = Circuit::self()->createItem(
        "SerialTerm", "SerialTerm-" + Circuit::self()->newSceneId());
    ser->setPos(pos());
    Circuit::self()->addItem(ser);
}
...
void McuComponent::slotReload()
{
    if (m_processor->getLoadStatus())
        ...
        QMessageBox::warning(0, tr("No File:"), tr("No File to reload "));
}
...
void McuComponent::loadData()
{
    ...
    bool resize = false;
    if (!m_processor->getLoadStatus())
        resize = true; // No eeprom initialized yet
...
void McuComponent::setLogicSymbol(bool ls)
{
    ...
    if (ls && m_pkgeFile.endsWith(".package"))
        m_pkgeFile.replace(".package", "_LS.package");
    if (!ls && m_pkgeFile.endsWith("_LS.package"))
        m_pkgeFile.replace("_LS.package", ".package");
...
void McuComponent::paint(QPainter *p, const QStyleOptionGraphicsItem *option,
                         QWidget *widget)
{
