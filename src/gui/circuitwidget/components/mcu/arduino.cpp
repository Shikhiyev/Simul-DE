...
{
    return new LibraryItem(tr("Arduino"), tr("Micro"), "arduinoUnoIcon.png",
                           "Arduino", Arduino::construct);
}
...
void Arduino::attach()
{
    ...
    } else if (enod != m_boardLedEnode) // Connected to external eNode: Delete
                                        // boardLed eNode
    {
        m_boardLedEnode = enod;
    } else
        return; // Already connected to boardLed eNode: Do nothing
...
void Arduino::initBoard()
{
    ...
    // Create board led
    m_boardLed =
        new LedSmd(this, "LEDSMD", m_id + "boardled", QRectF(0, 0, 4, 3));
    ...
    m_boardLed->getEpin(1)->setEnode(
        m_groundEnode); // Connect board led to ground

    if (objectName().contains("Mega"))
        m_boardLed->setPos(35 + 12, 125 + 105);
    else
    ...
    for (int i = 0; i < m_numpins; i++) // Create Pins
    {
        ...
        if (mcuPin->angle() == 0)
            mcuPin->move(-16, 0);
        else if (mcuPin->angle() == 180)
            mcuPin->move(16, 0);
        else if (mcuPin->angle() == 90)
            mcuPin->move(0, 32);
        else
            mcuPin->move(0, -320);
...
void Arduino::paint(QPainter *p, const QStyleOptionGraphicsItem *option,
                    QWidget *widget)
{
