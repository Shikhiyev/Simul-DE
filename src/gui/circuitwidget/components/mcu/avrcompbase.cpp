...
AvrCompBase::AvrCompBase(QObject *parent, QString type, QString id) :
    McuComponent(parent, type, id), m_avrI2C("avrI2C")
{
...
void AvrCompBase::attachPins()
{
    ...
    // ADC irq
    avr_irq_t *adcIrq =
        avr_io_getirq(cpu, AVR_IOCTL_ADC_GETIRQ, ADC_IRQ_OUT_TRIGGER);
    avr_irq_register_notify(adcIrq, adc_hook, this);

    if (m_sda && m_scl) {
        // I2C Out irq
        avr_irq_t *i2cOutIrq =
            avr_io_getirq(cpu, AVR_IOCTL_TWI_GETIRQ(0), TWI_IRQ_OUTPUT);
        avr_irq_register_notify(i2cOutIrq, i2c_out_hook, this);

        // TWEN change irq
        int twcrAddr = BaseProcessor::self()->getRegAddress("TWCR");
        if (twcrAddr < 0)
            qDebug() << "AvrCompBase::attachPins: TWCR Register Not found";
        else {
            ...
            m_i2cInIrq =
                avr_io_getirq(cpu, AVR_IOCTL_TWI_GETIRQ(0), TWI_IRQ_INPUT);
        }
    }
    m_attached = true;
}

void AvrCompBase::addPin(QString id, QString type, QString label, int pos,
                         int xpos, int ypos, int angle)
{
    if (m_initialized) {
        ...
        // qDebug()<<pos<<id<<label;
        AVRComponentPin *newPin =
            new AVRComponentPin(this, id, type, label, pos, xpos, ypos, angle);
        m_pinList.append(newPin);

        QString ty = getType(type, "adc");
        if (!ty.isEmpty())
            m_ADCpinList[ty.remove("adc").toInt()] = newPin;
...
void AvrCompBase::inStateChanged(int value)
{
    ...
    {
        uint32_t irqMsg =
            avr_twi_irq_msg(TWI_COND_READ, m_slvAddr, m_avrI2C.byteReceived());
        avr_raise_irq(m_i2cInIrq, irqMsg);
