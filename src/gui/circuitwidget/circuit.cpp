...
Circuit::Circuit(qreal x, qreal y, qreal width, qreal height,
...
Circuit::~Circuit()
{
    ...
    for (QDomDocument *doc : m_redoStack)
        delete doc;
    for (QDomDocument *doc : m_undoStack)
        delete doc;
    m_undoStack.clear();
    m_redoStack.clear();
...
QString Circuit::loc()
{
    ...
    if (m_lang == French)
        ...
        locale = "de";
    else if (m_lang == Russian)
        locale = "ru";
    else if (m_lang == Spanish)
        locale = "es";
    else if (m_lang == Pt_Brasil)
        locale = "pt_BR";
...
Pin *Circuit::findPin(int x, int y, QString id)
{
    ...
    for (QGraphicsItem *it : list) {
        ...
        if (pin && (pin->pinId().left(1) == id.left(1)))
            return pin;
    }
...
void Circuit::loadCircuit(QString &fileName)
{
    ...
    if (m_error != 0) {
        remove();
        for (Component *comp : m_compList)
            removeComp(comp); // Clean Nodes
    } else
        m_graphicView->centerOn(
            QPointF(1200 + itemsBoundingRect().center().x(),
                    950 + itemsBoundingRect().center().y()));
...
void Circuit::loadDomDoc(QDomDocument *doc)
{
    ...
    if (circuit.hasAttribute("speed"))
        setCircSpeed(circuit.attribute("speed").toInt());
    if (circuit.hasAttribute("reactStep"))
        setReactStep(circuit.attribute("reactStep").toInt());
    if (circuit.hasAttribute("noLinAcc"))
        setNoLinAcc(circuit.attribute("noLinAcc").toInt());
    if (circuit.hasAttribute("animate"))
        setAnimate(circuit.attribute("animate").toInt());
    ...
    while (!node.isNull()) {
        ...
        if (tagName == "item") {
            QString objNam =
                element.attribute("objectName"); // Data in simu file
            QString type = element.attribute("itemtype");
            QString id =
                objNam.split("-").first() + "-" + newSceneId(); // Create new id
            ...
            if (type == "Connector") {
                ...
                if (!startpin) // Pin not found by name... find it by pos
                {
                    QStringList pointList =
                        element.attribute("pointList").split(",");
                ...
                if (m_pasting) {
                    if (startpin && !startpin->component()->isSelected())
                        startpin = 0l;
                    if (endpin && !endpin->component()->isSelected())
                        endpin = 0l;
                }
                if (startpin && startpin->isConnected())
                    startpin = 0l;
                if (endpin && endpin->isConnected())
                    endpin = 0l;

                if (startpin && endpin) // Create Connector
                {
                    Connector *con =
                        new Connector(this, type, id, startpin, endpin);
                    ...
                    QStringList plist =
                        con->pointList(); // add lines to connector
                ...
                ; // TODO: this type shouldnt be saved to circuit file
                ...
                if (item) {
                    ...
                    if (docType != "simulide_0.4") {
                        if (type == "SerialPort")
                            item->setPos(McuComponent::self()->pos());
                    }
                    if (m_pasting)
    ...
    for (Node *joint : jointList)
        joint->remove(); // Only removed if some missing connector
...
void Circuit::circuitToDom()
{
    ...
    // circuit.setAttribute( "drawGrid",    QString(
    // drawGrid()?"true":"false")); circuit.setAttribute( "showScroll", QString(
    // showScroll()?"true":"false"));
...
void Circuit::importCirc(QPointF eventpoint)
{
    ...
    QString fileName  = QFileDialog::getOpenFileName(
        0l, tr("Load Circuit"), dir, tr("Circuits (*.simu);;All files (*.*)"));
    ...
    for (LibraryItem *libItem : ItemLibrary::self()->items()) {
        if (libItem->type() == type) {
            ...
            if (comp) {
                ...
                if ((category != "Meters") && (category != "Sources") &&
                    (category != "Other"))
...
void Circuit::loadObjectProperties(QDomElement element, QObject *Item)
{
    ...
    for (int i = 0; i < atrs.length();
         i++) // Get List of property names in Circuit file
    {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (metaproperty.type() == QVariant::Int)
            Item->setProperty(chName, value.toInt());
        else if (metaproperty.type() == QVariant::Double)
            Item->setProperty(chName, value.toDouble());
        else if (metaproperty.type() == QVariant::PointF)
            Item->setProperty(chName, value.toPointF());
        else if (metaproperty.type() == QVariant::Bool)
            Item->setProperty(chName, value.toBool());
            ...
            int lsize = list.size();
            vmem.resize(lsize);
            // qDebug() << "Circuit::loadObjectProperties eeprom size:" <<
            // lsize;
...
void Circuit::compRemoved(
    bool removed) // Arduino doesn't like to be removed while circuit is running
{
...
void Circuit::remove() // Remove everything ( Clear Circuit )
{
    ...
    while (!m_compList.isEmpty()) {
        ...
        comp->objectName().split("-").last().toInt(
            &isNumber); // TODO: Find a proper way !!!!!!!!!!!
...
void Circuit::saveChanges()
{
    ...
    if (!bckDir.isWritable())
        m_backupPath = SIMUAPI_AppPath::self()->RWDataFolder().absolutePath() +
                       "/_backup.simu";
...
void Circuit::paste(QPointF eventpoint)
{
    ...
    if (!clipText.contains("eventpoint"))
        return;
...
bool Circuit::pasting()
{
    return m_pasting;
}
QPointF Circuit::deltaMove()
{
    return m_deltaMove;
}
...
void Circuit::constarted(bool started)
{
    m_con_started = started;
}
bool Circuit::is_constarted()
{
    return m_con_started;
}

void Circuit::convertSubCircs() // Convert old subcircuits:
{
    QString subCirPath = QFileDialog::getExistingDirectory(
        0L, tr("Select Directory"), m_filePath,
    ...
    for (QString compName : compList) {
        ...
        if (!pkgFile.open(QFile::ReadOnly | QFile::Text)) {
            QMessageBox::warning(0, "Circuit::convertSubCircs",
                                 tr("Cannot read file %1:\n%2.")
                                     .arg(packag)
                                     .arg(pkgFile.errorString()));
            continue;
        }
        QDomDocument domDoc;
        if (!domDoc.setContent(&pkgFile)) {
            MessageBoxNB(
                "Circuit::convertSubCircs",
        ...
        for (int i = pinList.size(); i > 0; i--) {
            ...
            text.replace(QString("packagePin" + QString::number(i)),
                         idd); // replace text in string
        ...
        if (!fileS.open(QFile::WriteOnly | QFile::Text)) {
            QMessageBox::warning(0l, "Circuit::convertSubCircs",
                                 "Can't open file" + subName);
            continue;
        }
        ...
        QString pkgPath = subCirPath + "converted/" +
                          strippedName(packag); // Copy pkg file as well
...
void Circuit::createSubcircuit()
{
    ...
    fileName = QFileDialog::getSaveFileName(
        MainWindow::self(), tr("Create Subcircuit"), fileName, "All files (*)");

    QFileInfo fi(fileName);
    QString ext      = fi.suffix();
    QString filePath = fileName;
    if (!ext.isEmpty())
        filePath.remove(fileName.lastIndexOf(ext) - 1, ext.size() + 1);
    ...
    for (Component *component : m_compList) {
        ...
        for (int i = 0; i < count; ++i) // Get Properties
        {
            ...
            if (property.isUser()) {
                ...
                if (name.startsWith("_1"))
                    continue; // BcdToDec 16Bits property breaks DomDocument

                if (!name.contains("Show") && !name.contains("Unit") &&
                    !name.contains("itemtype")) {
                    QString valString = "";

                    if ((name == "Resistance") | (name == "Capacitance") |
                        (name == "Inductance") | (name == "Voltage") |
                        (name == "Current")) {
    ...
    for (eNode *node : eNodeList) // Get all the connections in each eNode
    {
        ...
        for (ePin *epin : pinList) {
            ...
            if (compType == "Package") {
                if (pin->inverted() && !pinId.startsWith("!"))
                    pinId = "!" + pinId;

                pinConList.prepend("Package_" + pinId);
            } else if ((compType == "Probe") || (compType == "Fixed Voltage")) {
                // Take care about "packagepin" bad spelling
                // compId = compId.replace( test.indexOf("packagepin"), 10,
                // "packagePin");
                pinConList.prepend("Package_" +
                                   component->idLabel().replace("-", ""));
        ...
        } else // Multiple connection
        {
            ...
            for (QString entry : pinConList) {
                // if( entry.contains("packagePin") ) // No Node, connection to
                // packagePin Package
                if (entry.contains(
                        "Package")) // No Node, connection to packagePin
                {
                    pin2 = entry;
...
void Circuit::bom()
{
    ...
    fileName = QFileDialog::getSaveFileName(
        MainWindow::self(), tr("Bill Of Materials"), fileName, "(*.*)");
...
void Circuit::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        ...
        if (m_con_started)
            event->accept(); // new_connector->incActLine() ;
...
void Circuit::keyReleaseEvent(QKeyEvent *event)
{
    if (!event->isAutoRepeat() &&
        !(event->modifiers() &
          Qt::ControlModifier)) // Deliver Key events ( switches )
    {
        QString keys = event->text();
...
void Circuit::dropEvent(QGraphicsSceneDragDropEvent *event)
{
    ...
    if (id.startsWith(file)) {
        ...
        if (loId.endsWith(".jpg") || loId.endsWith(".png") ||
            loId.endsWith(".gif")) {
            file                 = id;
            type                 = "Image";
            id                   = "Image";
            ...
            if (enterItem) {
                ...
                QPoint cPos = QCursor::pos() -
                              CircuitView::self()->mapToGlobal(QPoint(0, 0));
...
QList<Component *> *Circuit::compList()
{
    return &m_compList;
}
QList<Component *> *Circuit::conList()
{
    return &m_conList;
}
