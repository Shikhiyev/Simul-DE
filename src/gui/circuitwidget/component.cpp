...
Component::Component(QObject *parent, QString type, QString id) :
    QObject(parent), QGraphicsItem(), multUnits("TGMk munp")
{
    ...
    if ((type != "Connector") && (type != "Node")) {
        ...
        if (li) {
            if ((type == "Subcircuit") || (type == "AVR") || (type == "PIC") ||
                (type == "Arduino")) {
                QString name = id;
...
void Component::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (itemlist.size() > 1) {
        ...
        for (QGraphicsItem *item : itemlist) {
            ...
            if (comp && (comp->objectName() != "") &&
                (!comp->objectName().contains("Connector"))) {
                comp->move(delta);
            }
        }
...
void Component::updateLabelsTheme()
{
    bool dark = MainWindow::self()->isDarkMode();
    if (m_idLabel) {
        m_idLabel->setDefaultTextColor(dark ? Qt::cyan : Qt::darkBlue);
    }
    if (m_valLabel) {
        m_valLabel->setDefaultTextColor(dark ? Qt::white : Qt::black);
    }
}
...
QString Component::unit()
{
    return m_mult + m_unit;
}
    ...
    if (un.size() > 0) {
        ...
        for (int x = 0; x < 9; x++) {
            if (mul == multUnits.at(x)) {
                ...
                if (m_mult != " ")
                    m_mult.prepend(" ");
                m_valLabel->setPlainText(QString::number(m_value) + m_mult +
                                         m_unit);
                return;
            }
...
double Component::getmultValue()
{
    return m_value * m_unitMult;
}

bool Component::showId()
{
    return m_showId;
}
...
int Component::labelx()
{
    return m_idLabel->m_labelx;
}
void Component::setLabelX(int x)
{
    m_idLabel->m_labelx = x;
}

int Component::labely()
{
    return m_idLabel->m_labely;
}
void Component::setLabelY(int y)
{
    m_idLabel->m_labely = y;
}

int Component::labelRot()
{
    return m_idLabel->m_labelrot;
}
void Component::setLabelRot(int rot)
{
    m_idLabel->m_labelrot = rot;
}

int Component::valLabelx()
{
    return m_valLabel->m_labelx;
}
void Component::setValLabelX(int x)
{
    m_valLabel->m_labelx = x;
}

int Component::valLabely()
{
    return m_valLabel->m_labely;
}
void Component::setValLabelY(int y)
{
    m_valLabel->m_labely = y;
}

int Component::valLabRot()
{
    return m_valLabel->m_labelrot;
}
void Component::setValLabRot(int rot)
{
    m_valLabel->m_labelrot = rot;
}
...
QString Component::itemType()
{
    return m_type;
}
QString Component::category()
{
    return m_category;
}
QIcon Component::icon()
{
    return m_icon;
}
                      ...
                      QWidget *widget)
{
    ...
    QColor penColor = MainWindow::self()->isDarkMode() ? Qt::white : Qt::black;
    QPen pen(penColor, 1.5, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
...
Label::Label(Component *parent) : QGraphicsTextItem(parent)
{
    m_parentComp = parent;
    m_labelrot   = 0;
    setCursor(Qt::OpenHandCursor);
    ...
    connect(document(), SIGNAL(contentsChange(int, int, int)), this,
            SLOT(updateGeometry(int, int, int)));
...
{
    event->accept();
    setPos(pos() + mapToItem(m_parentComp, event->pos()) -
           mapToItem(m_parentComp, event->lastPos()));
    m_labelx = int(pos().x());
    m_labely = int(pos().y());
}
...
void Label::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    if (!acceptedMouseButtons())
        ...
        QAction *rotateCWAction =
            menu.addAction(QIcon(":/rotateCW.png"), "Rotate CW");
        connect(rotateCWAction, SIGNAL(triggered()), this, SLOT(rotateCW()));

        QAction *rotateCCWAction =
            menu.addAction(QIcon(":/rotateCCW.png"), "Rotate CCW");
        connect(rotateCCWAction, SIGNAL(triggered()), this, SLOT(rotateCCW()));

        QAction *rotate180Action =
            menu.addAction(QIcon(":/rotate180.png"), "Rotate 180º");
